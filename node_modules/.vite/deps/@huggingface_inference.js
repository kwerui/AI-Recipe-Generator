...
var BaseConversationalTask = class extends TaskProviderHelper {
  ...
  async getResponse(response) {
    if (typeof response === "object" && Array.isArray(response == null ? void 0 : response.choices) && typeof (response == null ? void 0 : response.created) === "number" && typeof (response == null ? void 0 : response.id) === "string" && typeof (response == null ? void 0 : response.model) === "string" && /// Some providers (e.g. Together.ai) do not output a system_fingerprint
...
// node_modules/@huggingface/inference/dist/esm/utils/dataUrlFromBlob.js
...
var HFInferenceTextGenerationTask = class extends HFInferenceTask {
  ...
  async getResponse(response) {
    var _a;
    if (typeof response === "object" && "choices" in response && Array.isArray(response.choices) && typeof ((_a = response.choices[0]) == null ? void 0 : _a.text) === "string") {
...
// node_modules/@huggingface/inference/dist/esm/providers/deepinfra.js
...
var DeepInfraFeatureExtractionTask = class extends TaskProviderHelper {
  ...
  async getResponse(response) {
    if (typeof response === "object" && response !== null && "data" in response && Array.isArray(response.data) && response.data.every((item) => typeof item === "object" && !!item && Array.isArray(item.embedding))) {
...
var FalAIImageTextToImageTask = class extends FalAIImageToImageTask {
  ...
  async preparePayloadAsync(args) {
    ...
    return {
      ...omit(args, ["inputs", "parameters"]),
      ...args.parameters,
      prompt: (_a = args.parameters) == null ? void 0 : _a.prompt,
...
var FalAIAutomaticSpeechRecognitionTask = class extends FalAITask {
  ...
  async getResponse(response) {
    const res = response;
    const text = typeof (res == null ? void 0 : res.text) === "string" ? res.text : typeof (res == null ? void 0 : res.output) === "string" ? res.output : void 0;
    if (typeof text !== "string") {
      throw new InferenceClientProviderOutputError(`Received malformed response from Fal.ai Automatic Speech Recognition API: expected { text: string } or { output: string } format, got instead: ${JSON.stringify(response)}`);
    }
    const output = { text };
    const chunks = Array.isArray(res.chunks) ? res.chunks.filter((c) => typeof (c == null ? void 0 : c.text) === "string" && Array.isArray(c.timestamp)).map((c) => ({ text: c.text, timestamp: c.timestamp })) : Array.isArray(res.segments) ? res.segments.filter((s) => typeof (s == null ? void 0 : s.text) === "string").map((s) => ({ text: s.text, timestamp: [s.start ?? 0, s.end ?? 0] })) : [];
    if (chunks.length > 0) {
...
var FalAITextToAudioTask = class extends FalAiQueueTask {
  ...
  async getResponse(response, url, headers, _outputType, signal) {
    ...
    if (typeof audio !== "object" || !audio || typeof audio.url !== "string" || !isUrl(audio.url)) {
      throw new InferenceClientProviderOutputError(`Received malformed response from Fal.ai text-to-audio API: expected { audio_file: { url: string } } or { audio: { url: string } } result format, got instead: ${JSON.stringify(result)}`);
    }
    const audioResponse = await fetch(audio.url, { signal });
    if (!audioResponse.ok) {
      throw new InferenceClientProviderApiError(`Failed to fetch audio from ${audio.url}: ${audioResponse.statusText}`, { url: audio.url, method: "GET" }, {
...
var FalAIAudioToAudioTask = class extends FalAiQueueTask {
  ...
  async preparePayloadAsync(args) {
    const blob = "data" in args && args.data instanceof Blob ? args.data : "inputs" in args ? args.inputs : void 0;
    if (!(blob instanceof Blob)) {
      throw new InferenceClientInputError(`Expected a Blob input for audio-to-audio with provider fal-ai, got ${typeof blob}.`);
    }
    return {
      ..."data" in args ? omit(args, "data") : omit(args, "inputs"),
      audio_url: await buildFalAiAudioDataUrl(blob)
  ...
  async getResponse(response, url, headers, _outputType, signal) {
    const result = await this.getResponseFromQueueApi(response, url, headers, signal);
    if (typeof result !== "object" || !result || typeof result.audio !== "object" || !result.audio || typeof result.audio.url !== "string" || !isUrl(result.audio.url)) {
      throw new InferenceClientProviderOutputError(`Received malformed response from Fal.ai audio-to-audio API: expected { audio: { url: string } } result format, got instead: ${JSON.stringify(result)}`);
    }
    const audioResponse = await fetch(result.audio.url, { signal });
    if (!audioResponse.ok) {
      throw new InferenceClientProviderApiError(`Failed to fetch audio from ${result.audio.url}: ${audioResponse.statusText}`, { url: result.audio.url, method: "GET" }, {
        requestId: audioResponse.headers.get("x-request-id") ?? "",
        status: audioResponse.status,
        body: await audioResponse.text()
      });
...
function mimeTypeToExtension2(mimeType) {
  if (!mimeType) {
    return "wav";
  }
  return AUDIO_MIME_TO_EXT2[mimeType.toLowerCase()] ?? "wav";
}
...
var TogetherImageToImageTask = class extends TogetherTextToImageTask {
  ...
  preparePayload(params) {
    ...
    const imageField = useImageUrl ? { image_url: params.args.inputs } : { reference_images: [params.args.inputs] };
    return {
      ...omit(params.args, ["inputs", "parameters"]),
      prompt: prompt ?? "",
      ...imageField,
...
function normalizeTogetherVideoParameters(parameters) {
  ...
  if (target_size && typeof target_size === "object") {
    if (target_size.width !== void 0) {
      rest.width = target_size.width;
    }
    if (target_size.height !== void 0) {
...
var TogetherVideoTask = class extends TaskProviderHelper {
  ...
  async getResponse(response, url, headers, _outputType, signal) {
    ...
    const jobId = response == null ? void 0 : response.id;
    if (!jobId) {
    ...
    if (status === "failed") {
      throw new InferenceClientProviderOutputError(`Together video generation failed: ${((_a = job.error) == null ? void 0 : _a.message) ?? "Unknown error"}`);
    }
    if (status !== "completed") {
      throw new InferenceClientProviderOutputError(`Unexpected Together video job status: ${JSON.stringify(status)}`);
    }
    const videoUrl = (_b = job.outputs) == null ? void 0 : _b.video_url;
...
var TogetherAutomaticSpeechRecognitionTask = class extends TaskProviderHelper {
  ...
  makeBody(params) {
    ...
    const formData = new FormData();
    if (audio instanceof Blob) {
      formData.append("file", audio, `audio.${mimeTypeToExtension2(audio.type)}`);
...
// node_modules/@huggingface/inference/dist/esm/providers/wavespeed.js
...
var WavespeedAITask = class extends TaskProviderHelper {
  ...
  preparePayload(params) {
    ...
    if (((_a = params.mapping) == null ? void 0 : _a.adapter) === "lora") {
      payload.loras = [
        {
  ...
  async getResponse(response, url, headers, outputType, signal) {
    ...
    while (true) {
      ...
      switch (taskResult.status) {
        case "completed": {
          if (!((_a = taskResult.outputs) == null ? void 0 : _a[0])) {
...
var WavespeedAIImageToImageTask = class extends WavespeedAITask {
  ...
  async preparePayloadAsync(args) {
    var _a, _b;
    const hasImages = args.images ?? ((_a = args.parameters) == null ? void 0 : _a.images);
    const { base, images } = await buildImagesField(args.inputs, hasImages);
    return { ...args, inputs: (_b = args.parameters) == null ? void 0 : _b.prompt, image: base, images };
  }
  async getResponse(response, url, headers, _outputType, signal) {
    return super.getResponse(response, url, headers, void 0, signal);
  }
...
var ZaiTextToImageTask = class extends ZaiTask {
  ...
  async getResponse(response, url, headers, outputType, signal) {
    ...
    if (typeof response !== "object" || !response || !("task_status" in response) || !("id" in response) || typeof response.id !== "string") {
    ...
    for (let attempt = 0; attempt < MAX_POLL_ATTEMPTS; attempt++) {
      ...
      if (result.task_status === "SUCCESS") {
        if (!result.image_result || !Array.isArray(result.image_result) || result.image_result.length === 0 || typeof ((_a = result.image_result[0]) == null ? void 0 : _a.url) !== "string" || !isUrl(result.image_result[0].url)) {
...
var ZaiImageToTextTask = class extends ZaiTask {
  ...
  async getResponse(response) {
    const mdResults = response == null ? void 0 : response.md_results;
    if (typeof mdResults !== "string") {
      throw new InferenceClientProviderOutputError(`Received malformed response from ZAI layout_parsing API: expected { md_results: string }, got: ${JSON.stringify(response)}`);
    }
...
async function audioToAudio(args, options) {
  ...
  return providerHelper.getResponse(res, url, info.headers, void 0, options == null ? void 0 : options.signal);
}
...
async function automaticSpeechRecognition(args, options) {
  ...
  return providerHelper.getResponse(res, void 0, void 0, void 0, options == null ? void 0 : options.signal);
}

// node_modules/@huggingface/inference/dist/esm/tasks/audio/textToAudio.js
...
async function imageToText(args, options) {
  ...
  const payload = await providerHelper.preparePayloadAsync(args, options == null ? void 0 : options.signal);
  const { data: res } = await innerRequest(payload, providerHelper, {
    ...options,
    task: "image-to-text"
  });
...
// node_modules/@huggingface/inference/dist/esm/tasks/cv/imageTextToImage.js
async function imageTextToImage(args, options) {
  ...
  return providerHelper.getResponse(res, requestContext.url, requestContext.info.headers, void 0, options == null ? void 0 : options.signal);
}

// node_modules/@huggingface/inference/dist/esm/tasks/cv/imageTextToVideo.js
...
async function textToImage(args, options) {
  ...
  return providerHelper.getResponse(res, url, info.headers, options == null ? void 0 : options.outputType, options == null ? void 0 : options.signal);
}
...
async function textToVideo(args, options) {
  ...
  return providerHelper.getResponse(response, url, info.headers, void 0, options == null ? void 0 : options.signal);
}
  ...
  main:
    while (cursorPosition < src.length) {
      ...
      if (isInteger(char)) {
        ...
        if (((_c = tokens.at(-1)) == null ? void 0 : _c.type) !== TOKEN_TYPES.Dot && src[cursorPosition] === "." && isInteger(src[cursorPosition + 1])) {
          ++cursorPosition;
...
var StringValue = class extends RuntimeValue {
  constructor() {
    ...
    __publicField(this, "_builtins");
  }
  get builtins() {
    return this._builtins ?? (this._builtins = /* @__PURE__ */ new Map([
      [
        "upper",
        new FunctionValue(() => {
...
var ArrayValue = class extends RuntimeValue {
  ...
  get builtins() {
    return this._builtins ?? (this._builtins = /* @__PURE__ */ new Map([["length", new IntegerValue(this.value.length)]]));
  }
  /**
   * NOTE: necessary to override since all JavaScript arrays are considered truthy,
   * while only non-empty Python arrays are consider truthy.
...
var _Environment = class {
  constructor(parent) {
    ...
    __publicField(this, "tests", _Environment.TESTS);
    this.parent = parent;
  }
  /**
...
__publicField2(Environment, "TESTS", /* @__PURE__ */ new Map([
...
var REMOVED_IN_V5_TRANSFORMERS_PIPELINES = ["image-to-text", "summarization", "translation"];
...
var data_default47 = taskData47;

// node_modules/@huggingface/tasks/dist/esm/tasks/video-to-video/data.js
var taskData48 = {
  datasets: [
    {
...
function getData(type, partialTaskData = data_default21) {
  return {
    ...partialTaskData,
    id: type,
...
var TASKS_DATA = {
  ...
  "depth-estimation": getData("depth-estimation", data_default20),
  "document-question-answering": getData("document-question-answering", data_default6),
  "visual-document-retrieval": getData("visual-document-retrieval", data_default39),
  ...
  "image-segmentation": getData("image-segmentation", data_default16),
  "image-to-image": getData("image-to-image", data_default11),
  "image-text-to-text": getData("image-text-to-text", data_default13),
  "image-text-to-image": getData("image-text-to-image", data_default14),
  "image-text-to-video": getData("image-text-to-video", data_default15),
  "image-to-text": getData("image-to-text", data_default12),
  "image-to-video": getData("image-to-video", data_default17),
  "keypoint-detection": getData("keypoint-detection", data_default46),
  "mask-generation": getData("mask-generation", data_default18),
  "multiple-choice": void 0,
  "object-detection": getData("object-detection", data_default19),
  "video-classification": getData("video-classification", data_default38),
  other: void 0,
  "question-answering": getData("question-answering", data_default23),
  "reinforcement-learning": getData("reinforcement-learning", data_default22),
  robotics: void 0,
  "sentence-similarity": getData("sentence-similarity", data_default24),
  summarization: getData("summarization", data_default25),
  "table-question-answering": getData("table-question-answering", data_default26),
  "table-to-text": void 0,
  "tabular-classification": getData("tabular-classification", data_default27),
  "tabular-regression": getData("tabular-regression", data_default28),
  "tabular-to-text": void 0,
  "text-classification": getData("text-classification", data_default33),
  "text-generation": getData("text-generation", data_default34),
  "text-ranking": getData("text-ranking", data_default35),
  "text-retrieval": void 0,
  "text-to-image": getData("text-to-image", data_default29),
  "text-to-speech": getData("text-to-speech", data_default30),
  "text-to-audio": void 0,
  "text-to-video": getData("text-to-video", data_default36),
  "time-series-forecasting": void 0,
  "token-classification": getData("token-classification", data_default31),
  translation: getData("translation", data_default32),
  "unconditional-image-generation": getData("unconditional-image-generation", data_default37),
  "video-text-to-text": getData("video-text-to-text", data_default47),
  "video-to-video": getData("video-to-video", data_default48),
  "visual-question-answering": getData("visual-question-answering", data_default40),
  "voice-activity-detection": void 0,
  "zero-shot-classification": getData("zero-shot-classification", data_default41),
  "zero-shot-image-classification": getData("zero-shot-image-classification", data_default42),
  "zero-shot-object-detection": getData("zero-shot-object-detection", data_default43),
  "text-to-3d": getData("text-to-3d", data_default45),
  "image-to-3d": getData("image-to-3d", data_default44)
};
...
model = AutoAdapterModel.from_pretrained("${escapeStringForJson(((_b = (_a = model.config) == null ? void 0 : _a.adapter_transformers) == null ? void 0 : _b.model_name) ?? "fill-in-model-name")}")
...
function get_base_diffusers_model(model) {
  var _a, _b;
  return escapeStringForJson(((_b = (_a = model.cardData) == null ? void 0 : _a.base_model) == null ? void 0 : _b.toString()) ?? "fill-in-base-model");
}
...
var keras_hub = (model) => {
  ...
  const tasks2 = (((_b = (_a = model.config) == null ? void 0 : _a.keras_hub) == null ? void 0 : _b.tasks) ?? []).filter(isValidIdentifier);
  const snippets2 = [];
...
var multimolecule = (model) => {
  ...
  const exampleText = escapeStringForJson((widgetExample == null ? void 0 : widgetExample.text) ?? "");
  const maskToken = model.mask_token ?? "<mask>";
  const sequence = exampleText == null ? void 0 : exampleText.replace(maskToken, "A");
  const snippets2 = [`pip install multimolecule`];
  if (sequence) {
    snippets2.push(`from multimolecule import AutoModel, AutoTokenizer
...
model = AutoModel.from_pretrained("${model.id}")`);
  ...
  if (model.tags.includes("rna-secondary-structure") && exampleText) {
    snippets2.push(`import multimolecule
...
var paddlenlp = (model) => {
  var _a, _b;
  const architecture = (_b = (_a = model.config) == null ? void 0 : _a.architectures) == null ? void 0 : _b[0];
...
var autoModelClass = (model) => {
  var _a;
  const autoModel = (_a = model.transformersInfo) == null ? void 0 : _a.auto_model;
...
var pruna_transformers = (model) => {
  ...
  if (auto_model) {
    processedSnippets = processedSnippets.map((snippet) => snippet.replace(new RegExp(`from transformers import ${auto_model}
...
(function(GGMLFileQuantizationType2) {
  ...
  GGMLFileQuantizationType2[GGMLFileQuantizationType2["NVFP4"] = 39] = "NVFP4";
  GGMLFileQuantizationType2[GGMLFileQuantizationType2["Q1_0"] = 40] = "Q1_0";
  GGMLFileQuantizationType2[GGMLFileQuantizationType2["Q2_0"] = 41] = "Q2_0";
...
(function(GGMLQuantizationType2) {
  ...
  GGMLQuantizationType2[GGMLQuantizationType2["NVFP4"] = 40] = "NVFP4";
  GGMLQuantizationType2[GGMLQuantizationType2["Q1_0"] = 41] = "Q1_0";
  GGMLQuantizationType2[GGMLQuantizationType2["Q2_0"] = 42] = "Q2_0";
...
// node_modules/@huggingface/tasks/dist/esm/hardware-nvidia.js
var NvidiaComputeCapabilities;
(function(NvidiaComputeCapabilities2) {
  NvidiaComputeCapabilities2[NvidiaComputeCapabilities2["BLACKWELL_ULTRA"] = 12.1] = "BLACKWELL_ULTRA";
  NvidiaComputeCapabilities2[NvidiaComputeCapabilities2["BLACKWELL_RTX"] = 12] = "BLACKWELL_RTX";
  NvidiaComputeCapabilities2[NvidiaComputeCapabilities2["BLACKWELL"] = 10] = "BLACKWELL";
  NvidiaComputeCapabilities2[NvidiaComputeCapabilities2["HOPPER"] = 9] = "HOPPER";
  NvidiaComputeCapabilities2[NvidiaComputeCapabilities2["ADA_LOVELACE"] = 8.9] = "ADA_LOVELACE";
  NvidiaComputeCapabilities2[NvidiaComputeCapabilities2["ORIN"] = 8.7] = "ORIN";
  NvidiaComputeCapabilities2[NvidiaComputeCapabilities2["AMPERE_RTX"] = 8.6] = "AMPERE_RTX";
  NvidiaComputeCapabilities2[NvidiaComputeCapabilities2["AMPERE"] = 8] = "AMPERE";
  NvidiaComputeCapabilities2[NvidiaComputeCapabilities2["TURING"] = 7.5] = "TURING";
  NvidiaComputeCapabilities2[NvidiaComputeCapabilities2["XAVIER"] = 7.2] = "XAVIER";
  NvidiaComputeCapabilities2[NvidiaComputeCapabilities2["VOLTA"] = 7] = "VOLTA";
  NvidiaComputeCapabilities2[NvidiaComputeCapabilities2["PASCAL_TEGRA"] = 6.2] = "PASCAL_TEGRA";
  NvidiaComputeCapabilities2[NvidiaComputeCapabilities2["PASCAL"] = 6.1] = "PASCAL";
  NvidiaComputeCapabilities2[NvidiaComputeCapabilities2["PASCAL_DATACENTER"] = 6] = "PASCAL_DATACENTER";
  NvidiaComputeCapabilities2[NvidiaComputeCapabilities2["MAXWELL"] = 5.3] = "MAXWELL";
...
var snippetGenerator = (templateName, inputPreparationFn) => {
  return (model, provider, inferenceProviderMapping, opts) => {
    ...
    const params = {
      ...
      task,
      directRequest: !!(opts == null ? void 0 : opts.directRequest)
    };
